...
var init_gene = () => {
    _gene_map = JSON.parse(gene_map_raw);
    gene_map = {}
    for (var key in _gene_map) {
        gene_map[_gene_map[key].index] = Object();
        gene_map[_gene_map[key].index].name = key;
        gene_map[_gene_map[key].index].sub = null;
        if(Object.keys(_gene_map[key]).length>1){
            gene_map[_gene_map[key].index].sub = _gene_map[key];
            delete gene_map[_gene_map[key].index].sub.index;
        }
    }
}